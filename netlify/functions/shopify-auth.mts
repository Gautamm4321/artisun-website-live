 ...
 * This function does that once and shows you the token so you can save it as
 * SHOPIFY_ADMIN_TOKEN in Netlify.
 *
 * It only runs while the Netlify variable SHOPIFY_AUTH_SETUP is set to "on".
 * Remove that variable when you're done — the function then returns 404.
 *
 * Needs in Netlify:
 ...
 *
 * App settings (Partner / Dev Dashboard, app version):
 *   App URL        https://YOUR-SITE/.netlify/functions/shopify-auth
 *   Redirect URL   https://YOUR-SITE/.netlify/functions/shopify-auth
...
export default async (req: Request) => {
  ...
  if (!clientId || !secret || !shopEnv) {
    return html('<h1>Setup incomplete</h1><p>Add SHOPIFY_ADMIN_DOMAIN, SHOPIFY_CLIENT_ID and SHOPIFY_CLIENT_SECRET in Netlify, redeploy, then try again.</p>', 500);
  }
  ...
  if (params.get('code')) {
    ...
    return html(
      `<h1>Done. Here is your permanent token</h1>
       ...
       <ol>
         <li>In Netlify → Site configuration → Environment variables, add <b>SHOPIFY_ADMIN_TOKEN</b> with this value.</li>
