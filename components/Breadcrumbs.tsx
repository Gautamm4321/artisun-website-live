...
export default function Breadcrumbs({ items }: { items: BreadcrumbItem[] }) {
  // Google Search ke liye standard BreadcrumbList Schema structure
  ...
  const schemaData = {
    ...
    itemListElement: items.map((item, index) => ({
      ...
      position: index + 1,
      name: item.label,
      item: item.href ? (item.href.startsWith('http') ? item.href : `${baseUrl}${item.href}`) : baseUrl,
    })),
  };

  return (
    <>
      {/* 1. Google Crawlers ke liye Backend Schema (Invisible) */}
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(schemaData) }}
      />

      {/* 2. Visual Breadcrumb UI (Visitors ke liye) */}
