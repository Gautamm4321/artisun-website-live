...
const BG_IMAGE_MOBILE = '/Origin second frame final.png';
  ...
  return (
    <div id="origin-why" className="origin-panel relative w-screen shrink-0 h-[100svh] scroll-mt-[105px] lg:scroll-mt-0 overflow-hidden">
      ...
      <div className="relative z-10 h-full w-full max-w-[1500px] mx-auto px-4 sm:px-8 md:px-10 lg:px-14 pt-20 pb-24 sm:pt-24 sm:pb-24 md:pt-20 md:pb-16 lg:pt-[104px] lg:pb-16 flex flex-col justify-between overflow-hidden">
        ...
        <div
          ref={statsRef}
          className="w-full grid grid-cols-2 md:grid-cols-5 lg:grid-cols-5 gap-2.5 sm:gap-3 md:gap-3 lg:gap-4 max-w-[340px] sm:max-w-[420px] md:max-w-none lg:max-w-none mx-auto shrink-0 mb-auto md:mb-0 lg:mb-0 pb-3 md:pb-2 lg:pb-0"
        >
          {STATS.map((s, idx) => {
            ...
            return (
              ...
              >
                <div>
                  {/* Category label */}
                  <div className="font-suisse text-[7.5px] sm:text-[8.5px] lg:text-[11px] tracking-[0.14em] uppercase text-[var(--brand-cream)]/70 lg:text-[var(--brand-cream)]/60">
                    {s.index} · {s.label}
                  </div>
                  {/* Big numbers on mobile */}
                  <div className="font-editorial text-[var(--brand-cream)] text-[30px] sm:text-[34px] lg:text-[60px] leading-none mt-1 lg:mt-2 tabular-nums drop-shadow-[0_2px_10px_rgba(0,0,0,0.7)]">
                    <CountUp end={s.value} suffix={s.suffix} play={inView} duration={2} />
                  </div>
                </div>

                {/* Subtext */}
                <p className="font-suisse text-[9.5px] sm:text-[10px] lg:text-[13.5px] leading-[1.25] lg:leading-[1.45] text-[var(--brand-cream)]/90 lg:text-[#E8DCC8] mt-0.5 lg:mt-2 line-clamp-2 lg:line-clamp-none drop-shadow-[0_1px_4px_rgba(0,0,0,0.8)]">
