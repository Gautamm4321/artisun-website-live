    ...
    <AnimatePresence>
      {open && (
        <motion.div
          key="signup-popup"
          data-lenis-prevent=""
          className="fixed inset-0 z-[200] flex items-end justify-center bg-black/55 sm:items-center sm:bg-black/65 sm:p-6"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
        ...
        >
          <motion.div
            ...
            className="relative flex max-h-[92svh] w-full flex-col overflow-hidden rounded-t-2xl bg-[#F4EDE3] text-[#180307] shadow-[0_-10px_40px_rgba(0,0,0,0.35)] sm:h-[540px] sm:max-h-none sm:max-w-[900px] sm:flex-row sm:rounded-none sm:shadow-[0_30px_80px_rgba(0,0,0,0.5)]"
          >
            {/* Image: full-width 4:3 on mobile, left column on desktop */}
            <div className="relative aspect-[4/3] w-full shrink-0 sm:aspect-auto sm:h-full sm:w-[380px]">
              <Image
                src={IMG_MOBILE}
            ...
            <div className="flex flex-1 flex-col justify-center overflow-y-auto px-6 pb-[max(20px,env(safe-area-inset-bottom))] pt-6 text-center font-suisse sm:h-full sm:px-[52px] sm:py-12 sm:text-left">
              {status === 'done' ? (
                <div className="flex flex-col gap-3">
                  ...
                  <p className="m-0 text-[13.5px] leading-[1.55] text-[#4a3a33] sm:text-[14.5px]">
                    Your first forecast is on its way. Watch your inbox.
                  </p>
                  <button
                    type="button"
                    onClick={close}
                    className="mt-2 h-[50px] w-full bg-[#A52A2C] text-[13px] font-medium tracking-[0.18em] text-[#FAF6EE] transition-colors hover:bg-[#8e2325]"
                  >
                    KEEP BROWSING
                  </button>
                </div>
                ...
                <>
                  <h2
                    id="signup-popup-title"
                    className="m-0 mb-[10px] font-editorial text-[28px] font-normal leading-[1.05] tracking-[-0.01em] sm:mb-[14px] sm:text-[40px]"
                  >
                    Your skin&rsquo;s forecast,
                    <br />
                    in your inbox.
                  </h2>
                  <p className="m-0 mb-5 text-[14px] leading-[1.55] text-[#4a3a33] sm:mb-[22px] sm:text-[14.5px]">
                    {/* Shorter copy on mobile, full copy on desktop */}
                    <span className="sm:hidden">
                      Sign up and we&rsquo;ll help you wear sunscreen right. Plus first access to everything new.
                    </span>
                  ...
                  <form onSubmit={submit} noValidate className="flex flex-col gap-[10px]">
                    ...
                    <input
                      ...
                      className="h-12 w-full rounded-none border border-[#cbbba9] bg-white px-[14px] text-[15px] text-[#180307] outline-none transition-colors placeholder:text-[#8a7a70] focus:border-[#180307]"
                    />
                    ...
                    {error && (
                      <p id="signup-popup-error" role="alert" className="m-0 text-[13px] text-[#A52A2C]">
                        {error}
                      </p>
                    )}
                    ...
                    <button
                      ...
                      className="mt-1 h-[50px] w-full bg-[#A52A2C] text-[13px] font-medium tracking-[0.18em] text-[#FAF6EE] transition-colors hover:bg-[#8e2325] disabled:opacity-70"
                    >
                      {status === 'sending' ? 'SIGNING YOU UP…' : 'START NOW'}
                    </button>
                  </form>

                  <p className="m-0 mt-4 text-[12px] leading-[1.5] text-[#6e5e55]">
                    By signing up you agree to receive marketing emails from Artisun. Unsubscribe anytime. See
                    our{' '}
                    <Link href="/privacy" className="underline underline-offset-2 hover:text-[#A52A2C]">
                      privacy policy
                    </Link>
                    .
                  </p>
            ...
            <button
              ...
              onClick={close}
              aria-label="Close"
              className="absolute right-3 top-3 flex h-11 w-11 items-center justify-center bg-transparent text-[#F4EDE3] drop-shadow-[0_1px_2px_rgba(0,0,0,0.4)] transition-opacity hover:opacity-80 sm:right-2 sm:top-2 sm:text-[#180307] sm:drop-shadow-none"
            >
